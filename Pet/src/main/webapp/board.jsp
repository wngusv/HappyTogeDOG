...
.custom-button-color {
	...
	border-color: rgb(88, 184, 117); /* 버튼 테두리 색상도 같게 설정 */
...
<body style="padding-top: 150px; background-color: rgb(254, 247, 222);">
		...
		<div class="container">
			<section class="strays-info">
				...
				<form action="board.jsp" method="get">
					...
					<input type="submit" class="btn btn-sm custom-button-color"
						value="검색" style="margin-bottom: 6px; margin-left: 6px">
				</form>
				...
				<table class="table table-striped table-hover fixed-layout-table">
					...
					<tbody>
						...
						String sql = "SELECT b.idx, b.category, b.title, b.id, b.postdate, "
								...
								+ "LEFT JOIN comment_content cc ON b.idx = cc.post_idx " + "WHERE b.town = ? ";
 ...
 %>
				...
				try (Connection conn = MyWebContextListener.getConnection(); PreparedStatement stmt = conn.prepareStatement(countSql)) {
					...
					// 이전 페이지 링크
					if (pagee > 1) {
						out.println("<li class='page-item'><a class='page-link' href='board.jsp?pagee=" + (pagee - 1)
								+ "'>&laquo;</a></li>");
					}
					...
					for (int i = 1; i <= totalPages; i++) {
						...
						if (i == pagee) {
							out.println("<li class='page-item active'><a class='page-link' href='" + pageLink + "'>" + i
									+ "</a></li>");
						} else {
							out.println(
